function mesh = rg_grid_midvoxel(cfg, mri)

% TODO: 

% based on PREPARE_MESH_HEXAHEDRAL
%
...
% ########## this part could be skipped probably ##########
%{
% ##########
...
cfg.resolution  = ft_getopt(cfg, 'resolution', 1);  % this is in mm
cfg.shift       = ft_getopt(cfg, 'shift');
cfg.background  = ft_getopt(cfg, 'background', 1);
...
disp('########## Function rg_prepare_mesh_hexahedral has been commented from line 64 to 98 ##########')
...
disp('########## Function rg_prepare_mesh_hexahedral has been commented from line 103 to 140 ##########')
% if (cfg.resolution ~= 1)
...
% build the mesh
mesh = build_mesh_hexahedral(cfg, mri);
...
disp('########## Function prepare_mesh_hexahedral commented from line 185 to 195 ##########')  % ADDED
...
disp('########## Function prepare_mesh_hexahedral changed between lines 194 and 197 #####')  % ADDED
x_dim = mri.dim(1);  % ADDED
y_dim = mri.dim(2);  % ADDED
z_dim = mri.dim(3);  % ADDED

fprintf('Dimensions of the segmentation after restriction to bounding-box: %i %i %i\n', x_dim, y_dim, z_dim);

% create elements
disp('########## Function create_elements inside rg_prepare_mesh_hexahedral.m has been commented ##########') % ADDED
disp('Function prepare_mesh_hexahedral changed from line 204 to 207')  % ADDED
...
% ########## I'm skipping this block by setting shift to 0 ##########
...
disp('########## Function prepare_mesh_hexahedral commented from line 222 to 229 ##########')  % ADDED
% background = 1;  % ADDED
...
disp('########## Function prepare_mesh_hexahedral changed between line 229 and 236 ##########')  % ADDED
%{
% delete unused nodes
...
disp('########## Function create_nodes inside rg_prepare_mesh_hexahedral.m has been changed ##########')
